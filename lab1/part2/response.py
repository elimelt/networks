...
# Initialize a Header for each server packet to be sent
class Header:
...
# Initialize and process a server packet to be sent
class Response:
    def __init__(self, header=None, payload=b""):
        self.header: Header = header
        self.payload: bytes = payload

    # add payload of type bytes literal
    def add_payload(self, payload: bytes):
        self.payload = payload

    # add header of type Header
    def add_header(self, header: Header):
        self.header = header

    # pack response to network bytes with the below format specifiers: 
